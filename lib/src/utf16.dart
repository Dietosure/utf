...
List<int?> encodeUtf16be(String str, [bool writeBOM = false]) {
  ...
  var encoding = List<int?>.filled(
      2 * utf16CodeUnits.length + (writeBOM ? 2 : 0), null,
      growable: false);
  var i = 0;
  if (writeBOM) {
...
abstract class Utf16BytesToCodeUnitsDecoder implements ListRangeIterator {
  ...
  List<int?> decodeRest() {
    var codeunits = List<int?>.filled(remaining, null, growable: false);
    var i = 0;
    while (moveNext()) {
      codeunits[i++] = current;
    }
    if (i == codeunits.length) {
      ...
      var truncCodeunits = List<int?>.filled(i, null, growable: false);
