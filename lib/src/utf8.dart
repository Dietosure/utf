...
List<int?> codepointsToUtf8(List<int?> codepoints,
  ...
  var encoded = List<int?>.filled(encodedLength, null, growable: false);
  var insertAt = 0;
  for (var value in source) {
    if (value! < 0 || value > UNICODE_VALID_RANGE_MAX) {
...
class Utf8Decoder implements Iterator<int?> {
  ...
  //into a [List<int>].
  List<int?> decodeRest() {
    var codepoints = List<int?>.filled(utf8EncodedBytesIterator.remaining, null,
        growable: false);
    var i = 0;
    while (moveNext()) {
    ...
    if (i == codepoints.length) {
      ...
      var truncCodepoints = List<int?>.filled(i, null, growable: false);
